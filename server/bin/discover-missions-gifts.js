...
var dataTableNames = [
'gifts','missions','logs_redeem','gift_redeem','mission_progress','view_distinct_mission_progress'
];
...
function discoverAndBuild(tableName,callback){
	...
	dataSource.discoverSchema(tableName,{schema: 'sans_taxigo'},function(err,schema){
		if (err) {
			...
			// Write JSON Config!!
			var jsonFilePath = path.resolve(__dirname, '../../common/models/gifts-and-missions/'+tableName+'.json');
			var existingJson;
			...
			var jsFilePath = path.resolve(
						__dirname, '../../common/models/gifts-and-missions/'+tableName+'.js'
					);
